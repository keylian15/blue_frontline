...
from src.units.IA.ChaloupeAI import ChaloupeAI
...
class Chaloupe(Unit):
    ...
    def pos_cible_ennemie(self, all_units: list) -> tuple:
        ...
        for unit in all_units:
            ...
            if unit.team == self.team or not unit.is_alive or getattr(unit, "is_platform", False):
                continue

            # Calculer la distance
            distance = math.sqrt(
                (unit.position[0] - self.position[0]) ** 2 + (unit.position[1] - self.position[1]) ** 2
            )
    ...
    def direct_follow_target(self):
        ...
        distance_to_target = math.sqrt(
            (current_target_position[0] - self.position[0]) ** 2 + (current_target_position[1] - self.position[1]) ** 2
        )
    ...
    def is_point_in_quantum_island(self, x: float, y: float) -> bool:
        ...
        for island in self.game.quantum_islands:
            if hasattr(island, "matrix") and hasattr(island, "rect"):
                ...
                if island.rect.x <= x < island.rect.right and island.rect.y <= y < island.rect.bottom:
                    ...
                    if 0 <= grid_y < len(island.matrix) and 0 <= grid_x < len(island.matrix[grid_y]):
    ...
    def draw_ai_debug(self, screen: pygame.Surface, camera_offset: tuple):
        ...
        try:
            ...
            if qlearning_info.get("enabled", False):
                ...
                reward_color = (
                    (0, 255, 0) if last_reward > 0 else (255, 100, 100) if last_reward < 0 else (200, 200, 200)
                )
