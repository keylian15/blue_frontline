...
import pygame
from src.system.Combat import CombatSystem, Mine
...
class SousMarin(Unit):
    ...
    def can_place_mine(self):
        ...
        multiplica = (
            self.game.hud.timer.get_speed_multiplier()
    ...
    def is_path_clear(
        ...
        target_y,
        num_checks=10,
        treat_platform_as_obstacle: bool = False,
    ):
    ...
    def form_attack_group(self, all_units, detected_target=None, radius=600, min_members=2):
        ...
        nearby = [
            a
            for a in allies
            if a != self and math.hypot(a.position[0] - self.position[0], a.position[1] - self.position[1]) <= radius
        ]
            ...
            if hasattr(unit, "team") and unit.team == self.team:
                ...
                if hasattr(unit, "unit_type"):
                    ...
                    if unit.unit_type in [
                        "plateforme",
                        "platform",
                        "plateforme_petroliere",
                        "PlateformePetroliere",
                    ]:
                        is_platform = True
    ...
    def return_to_base(self, all_units):
        ...
        if self.platform_position:
            ...
            # 2) Si on doit recalculer ou s'il n'y a pas de chemin, démarrer/attendre le thread
                ...
                if (
                    getattr(self, "path_thread", None)
                    and not self.path_thread.is_alive()
                    and not self.path_found
    ...
    def ia_behavior_patrol(self, all_units):
        ...
        if nearby_chaloupes:
            ...
            if len(allies) >= 2:
                formed = self.form_attack_group(
                    all_units,
                    detected_target=nearby_chaloupes[0],
                    radius=600,
                    min_members=3,
                )
                if formed:
        ...
        if nearby_bateaux:
            ...
            if len(allies) >= 2:
                formed = self.form_attack_group(
                    all_units,
                    detected_target=nearby_bateaux[0],
                    radius=600,
                    min_members=3,
                )
                if formed:
        ...
        if nearby_paquebots:
            ...
            if len(allies) >= 2:
                formed = self.form_attack_group(
                    all_units,
                    detected_target=nearby_paquebots[0],
                    radius=600,
                    min_members=3,
                )
                if formed:
