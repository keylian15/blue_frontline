...
class QLearningAgent:
    ...
    def get_state(self, chaloupe, all_units: list = None) -> tuple:
        ...
        if all_units:
            ...
            if closest_enemy:
                distance = math.sqrt(
                    (chaloupe.position[0] - closest_enemy.position[0]) ** 2
                    + (chaloupe.position[1] - closest_enemy.position[1]) ** 2
                )
    ...
    def calculate_reward(
        ...
        if all_units:
            _enemy_before = previous_state[2] if len(previous_state) > 2 else "none"
    ...
    def _find_closest_enemy(self, chaloupe, all_units: list) -> Any | None:
        ...
        for unit in all_units:
            if unit.team != chaloupe.team and unit.is_alive:
                distance = math.sqrt(
                    (chaloupe.position[0] - unit.position[0]) ** 2 + (chaloupe.position[1] - unit.position[1]) ** 2
                )
