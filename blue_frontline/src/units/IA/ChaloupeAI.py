...
try:
    from src.units.IA.ChaloupeQLearning import CHALOUPE_ACTIONS, QLearningAgent
...
class ChaloupeAI:
    ...
    def _handle_searching(self, all_units: list):
        ...
        if best_target:
            ...
            self._change_state(ChaloupeState.POSITIONING, f"Cible trouvée: {best_target.unit_type} à {int(distance)}px")
        ...
        if self._can_attack_target():
            ...
            if hasattr(self.unit.game, "combat_system"):
                try:
                    ...
                    if projectile:
                        self.log_decision("PROJECTILE TIRÉ avec succès!")
                    else:
                        self.log_decision("Attaque échouée - projectile non créé")
    ...
    def _handle_retreat(self):
        ...
        if retreat_duration >= self.min_retreat_duration:
            self._change_state(
                ChaloupeState.POSITIONING,
                "Distance sécurisée atteinte - Repositionnement",
            )
    ...
    def _find_best_target(self, all_units: list) -> object:
        ...
        for unit in all_units:
            ...
            if unit.team == self.unit.team or not unit.is_alive:
                continue
    ...
    def _can_attack_target(self) -> bool:
        ...
        if can_attack:
            self.log_decision(
                f"Distance à cible: {distance:.0f}px, Portée chaloupe: {attack_range}px, Peut attaquer: {can_attack}"
            )
    ...
    def _calculate_orbit_position(self, angle_override: float | None = None) -> tuple:
    ...
    def _evasive_positioning(self):
        ...
        if self.unit.need_pathfinding_to_target(evasive_pos):
            ...
            if current_time - self.unit.last_pathfinding_time > self.unit.pathfinding_cooldown:
                ...
                self.unit.start_pathfinding_thread(lambda: self.unit.compute_path_to_target(evasive_pos))
        else:
    ...
    def _dynamic_orbit(self):
        ...
        if self.unit.need_pathfinding_to_target(orbit_pos):
            ...
            if current_time - self.unit.last_pathfinding_time > self.unit.pathfinding_cooldown:
                ...
                self.unit.last_pathfinding_time = current_time
                self.unit.start_pathfinding_thread(lambda: self.unit.compute_path_to_target(orbit_pos))
        else:
        ...
        if distance > 0:
            ...
            if self.unit.need_pathfinding_to_target(retreat_pos):
                ...
                if current_time - self.unit.last_pathfinding_time > self.unit.pathfinding_cooldown:
                    ...
                    self.unit.last_pathfinding_time = current_time
                    self.unit.start_pathfinding_thread(lambda: self.unit.compute_path_to_target(retreat_pos))
            else:
    ...
    def _move_to_safe_position(self):
        ...
        if self.unit.need_pathfinding_to_target(safe_pos):
            ...
            if current_time - self.unit.last_pathfinding_time > self.unit.pathfinding_cooldown:
                ...
                self.unit.last_pathfinding_time = current_time
                self.unit.start_pathfinding_thread(lambda: self.unit.compute_path_to_target(safe_pos))
        else:
    ...
    def _move_directly_to_target(self):
        ...
        if self.unit.need_pathfinding_to_target(target_pos):
            ...
            if current_time - self.unit.last_pathfinding_time > self.unit.pathfinding_cooldown:
                ...
                self.unit.last_pathfinding_time = current_time
                self.unit.start_pathfinding_thread(lambda: self.unit.compute_path_to_target(target_pos))
        else:
    ...
    def get_debug_info(self) -> dict[str, Any]:
        ...
        try:
            debug_info = {
                ...
                "distance_to_target": int(self._calculate_distance(self.unit.position, self.target.position))
    ...
    def _update_qlearning(self, all_units: list):
        ...
        if self.previous_state is not None and self.current_action is not None:
            ...
            self.qlearning_agent.update_q_table(self.previous_state, self.current_action, reward, self.current_state)
        ...
        try:
            ...
            if action == "move_to_enemy":
                ...
                if self.target and self.state == ChaloupeState.POSITIONING:
                    self._dynamic_orbit()
                    self.log_decision("Q-Learning: Orbite dynamique appliquée")
                ...
                if self.target and self._can_attack_target() and self.state == ChaloupeState.POSITIONING:
                    ...
                    self._change_state(
                        ChaloupeState.POSITIONING,
                        "Q-Learning: Positionnement pour attaque",
                    )

            elif action == "change_angle":
                if self.state == ChaloupeState.POSITIONING:
                    self._rotate_orbit_angle()
                    self.log_decision("Q-Learning: Changement d'angle appliqué")

            elif action == "hide":
                # Pour l'instant, considérer comme une retraite vers un obstacle
                if self.state == ChaloupeState.POSITIONING:
                    self._change_state(ChaloupeState.RETREAT, "Q-Learning: Se cacher (retraite)")

            elif action == "wait":
                # Maintenir la position actuelle, pas de changement d'état
                self.log_decision("Q-Learning: Attendre en position")
    ...
    def toggle_qlearning(self, enabled: bool):
        ...
        if QLEARNING_AVAILABLE:
            ...
            print("[ChaloupeAI] Q-Learning non disponible")
