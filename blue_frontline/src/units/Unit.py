...
import pygame
from src.config.paths import GREEN_TEAM_PATH, RED_TEAM_PATH
...
from src.utils.Utils import load_tileset, point_in_many_polygons, random_point_in_polygon
...
class Unit(pygame.sprite.Sprite):
    ...
    def check_area(self, dt: int):
        ...
        # Si la prochaine position est la position d'une entité autre que la notre,
        ...
        if result:
            if self.type == "eclaireur":
                ...
                self.game.quantique_area_name.append(
                    "ile_quantique_" + str(index)
                )  # On ajoute le nom de l'ile dans la liste des iles quantiques

                self.game.renderer.refresh_map()  # On rafraichit le rendu
                self.game.quantique(
                    "ile_quantique_" + str(index)
                )  # On appel la fonction quantique du jeu pour activer le changement
            else:
        ...
        # On vérifie si la prochaine position est dans une zone quantique découverte
        # (Il suffit juste de regarder l'ensemble des zones)
        ...
        # On met a jour la vitesse de l'unité, speed est la valeur de déplacement,
        # move_to va redefinir la vitesse de déplacement jusqu'a la cible
        if self.is_moving:
    ...
    def move(self, dt: int):
        ...
        if self.game.hud.timer.get_speed_multiplier() >= 10:
            ...
            if self.is_moving:
                ...
                if (to_target_before[0] * to_target_after[0] + to_target_before[1] * to_target_after[1]) <= 0:
                    self.stop()
                    self.is_moving = False
                    self.target_position = None
        else:
    ...
    def take_damage(self, damage: int, killer: Unit = None):
        ...
        if self.current_health <= 0:
            ...
            if hasattr(self, "game") and hasattr(self.game, "combat_system"):
                from src.system.Combat import Explosion
    ...
    def die(self, killer: Unit = None):
        ...
        if (
            killer
            and hasattr(killer, "team")
            and killer.team != self.team
            and hasattr(self, "game")
            and hasattr(self.game, "hud")
        ):
    ...
    def draw_health_bar(
        ...
        Args:
            ...
            camera_offset (tuple[int, int], optional): La postition de la caméra.
    ...
    def draw_range(self, screen: pygame.Surface, camera_offset: list[int, int] = (0, 0)):
        ...
        if (
            -range_radius <= center_x <= screen.get_width() + range_radius
            and -range_radius <= center_y <= screen.get_height() + range_radius
        ):
            ...
            if self.enemies_in_range:
                ...
                text = font.render(
                    f"Ennemis: {len(self.enemies_in_range)} - Appuyez sur 'T'",
                    True,
                    (255, 255, 255),
                )
    ...
    def update_enemies_in_range(self, all_units: list[Unit]):
        ...
        for unit in all_units:
            ...
            # Si la cible a un rect (plateforme ou autre sprite),
            # on fait une collision cercle-rectangle
    ...
    def get_closest_enemy_in_range(self):
        ...
        for enemy in self.enemies_in_range:
            ...
            distance = math.sqrt(
                (self.position[0] - enemy.position[0]) ** 2 + (self.position[1] - enemy.position[1]) ** 2
            )
