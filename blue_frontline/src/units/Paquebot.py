...
class Paquebot(Unit):
    ...
    def pos_base_ennemie(self):
        ...
        if self.team == "red":
            return (
                self.game.plateformes["green"].position,
                self.game.plateformes["green"],
            )
        ...
        if allied_pos:
            ...
            for u in units:
                if getattr(u, "team", None) and u.team != self.team and getattr(u, "is_alive", True):
                    ...
                    d_self_to_base = math.hypot(
                        self.position[0] - allied_pos[0],
                        self.position[1] - allied_pos[1],
                    )
                        ...
                        if d_enemy_to_base < d_self_to_base + 8:
                            ...
                            if not (self.path_thread and self.path_thread.is_alive()):
                                self.start_pathfinding_thread(
                                    lambda allied_pos=allied_pos: self.compute_path_to(allied_pos)
                                )
                            return
                        else:
                            self.target = u
                            if not (self.path_thread and self.path_thread.is_alive()):
                                self.start_pathfinding_thread(lambda u_pos=u_pos: self.compute_path_to(u_pos))
                            return
