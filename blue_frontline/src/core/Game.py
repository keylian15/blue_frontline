...
from src.core.GameUpdater import GameUpdater
from src.core.Renderer import Renderer
from src.core.Timer import Timer
from src.system.AchievementsNotification import AchievementNotificationManager
...
from src.system.EventHandler import EventHandler
from src.system.Perlin import Perlin
from src.system.Petrole import Petrole
from src.system.Piece import Piece
from src.units.IA.IA_Eclaireur import SimpleGrid, make_grid_adapter_from_simplegrid
from src.units.PlateformePetroliere import PlateformePetroliere
...
class Game:
    ...
    def __init__(self, screen: pygame.surface, mode: str = "normal"):
        ...
        if self.mode == "tuto":
            ...
            from src.tutorial.TutorialManager import TutorialManager
    ...
    def setObstacles(self):
        ...
        layer = next((layer_obj for layer_obj in self.tmx_data.layers if layer_obj.name == layer_name), None)
        if not layer:
            return
    ...
    def quantique(self, name: str = None):
        ...
        def create_island(obj: TiledObject):
            """Fonction interne pour créer une île quantique.

            Args:
                obj (TiledObject): L'objet
            """
    ...
    def on_platform_destroyed(self, platform: PlateformePetroliere):
        ...
        if self.winner_team == "red" and hasattr(self, "achievements_system_rouge") and self.achievements_system_rouge:
            ...
            self.winner_team == "green" and hasattr(self, "achievements_system_vert") and self.achievements_system_vert
        ):
    ...
    def draw_victory_screen(self):
        ...
        mouse_on_restart = (
            restart_button_x <= mouse_pos[0] <= restart_button_x + button_width
            and restart_button_y <= mouse_pos[1] <= restart_button_y + button_height
        )
        ...
        mouse_on_menu = (
            menu_button_x <= mouse_pos[0] <= menu_button_x + button_width
            and menu_button_y <= mouse_pos[1] <= menu_button_y + button_height
        )
