...
class Renderer:
    ...
    def render_selected_unit_highlight(self):
        ...
        if (
            -50 <= unit_screen_x <= self.game.screen.get_width() + 50
            and -50 <= unit_screen_y <= self.game.screen.get_height() + 50
        ):
            # Cercle de portée rouge
            range_radius = (
                self.game.selected_unit.range * 32 * self.game.camera.zoom_level
            )  # Portée en pixels avec zoom
            ...
            self.game.screen.blit(
                range_surface,
                (unit_screen_x - range_radius, unit_screen_y - range_radius),
            )
            ...
            pygame.draw.circle(
                ...
                (255, 255, 0),
                (int(unit_screen_x), int(unit_screen_y)),
                int(pulse_scaled + 8 * self.game.camera.zoom_level),
                3,
            )
            pygame.draw.circle(
                self.game.screen,
                (255, 255, 0),
                (int(unit_screen_x), int(unit_screen_y)),
                int(18 * self.game.camera.zoom_level),
                2,
            )
            pygame.draw.circle(
                self.game.screen,
                (255, 255, 0),
                (int(unit_screen_x), int(unit_screen_y)),
                int(3 * self.game.camera.zoom_level),
                0,
            )
            ...
            if hasattr(self.game.selected_unit, "enemies_in_range") and self.game.selected_unit.enemies_in_range:
                ...
                controls_keys = get_controls_keys()  # S'assurer que les contrôles sont chargés
