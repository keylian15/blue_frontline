...
class TutorialManager:
    ...
    def __init__(self, game):
        ...
        self.messages = [
            {
                "message": "Bienvenue dans le didactitiel de Blue Frontline ! Je suis le Commandant Capitaine,\
                    votre guide pour vous préparer a cette guerre navale. Je vais vous expliquer comment jouer.",
            ...
            {
                "message": "Commençons par les bases ! Vous pouvez quitter le didactitiel à tout moment en appuyant\
                    sur la touche Echap.",
                "sub_text": default_sub_text,
                "anchor_pos": pos_center,
                "orientation": "hg",
            ...
            {
                "message": "Dans ce jeu, plusieurs modes de jeu sont disponibles.\
                    Vous pouvez jouer en solo contre l'IA, en multijoueur local \
                        (en utilisant la touche J pour changer d'équipe) ou même IA contre IA. Tout est possible ! \
                            Je vous laisserais jetter un oeil aux paramètres plus tard.",
                "sub_text": default_sub_text,
                "anchor_pos": pos_center,
                "orientation": "hg",
                "arrow": False,
            ...
            {
                "message": "Et ici les pièces obtenues par les morts,\
                    elle vous permettra d'améliorer la plateforme pétrolière.",
                "sub_text": default_sub_text,
            ...
            },
            {
                ...
                "restriction": {
                    "name": "zqsd",
                    "keys": [
                        get_action_key("CAMERA_UP"),
                        get_action_key("CAMERA_DOWN"),
                        get_action_key("CAMERA_LEFT"),
                        get_action_key("CAMERA_RIGHT"),
                    ],
                },
            },
            {
                ...
                "restriction": {
                    "name": "zoom",
                    "keys": [get_action_key("ZOOM_IN"), get_action_key("ZOOM_OUT")],
                },
            },
            {
                ...
                "restriction": {
                    "name": "navigation",
                    "keys": [get_action_key("HUD_LEFT"), get_action_key("HUD_RIGHT")],
                },
            },
            {
                ...
                "count": 0,
                "restriction": {
                    "name": "select_chaloupe",
                    "keys": [get_action_key("SELECT_MOVE")],
                },
            },
            {
                ...
                "count": 0,
                "function": [
                    {"name": self.move_to_base, "args": ("red",)},
                    {"name": self.zoom_in, "args": None},
                    {"name": self.set_oil_tuto, "args": ("20",)},
                ],
                "restriction": {
                    "name": "spawn_chaloupe",
                    "keys": [get_action_key("CREATE_UNIT")],
                },
            },
            {
            ...
            },
            {
                ...
                "count": 0,
                "function": [
                    {"name": self.do_seq_submarin, "args": None},
                    {"name": self.set_oil_tuto, "args": ("180",)},
                ],
            ...
            },
            {
                ...
                "function": [
                    {
                        "name": self.move_cam_to,
                        "args": ((self.width * 1.2, self.height * 0.5),),
                    },
                    {"name": self.unselect_unit, "args": None},
                ],
            },
            {
                "message": "Des zones quantiques, mais c'est une longue histoire,\
                    ne me refaites pas me souvenir de ca...",
            ...
            {
                "message": "Maintenant, vous savez tout. Allez, allez, allez ! \
                    Vous pouvez maintenant vous aventurer dans le monde de Blue Frontline !",
        ...
        if self.do_function and "function" in self.messages[self.step]:
            for function in self.messages[self.step]["function"]:
                ...
                if fn == "alien":
                    ...
                    self.message = self.messages[self.step]["message"].replace("{ALIEN}", alien)
                    self.sub_text = self.messages[self.step]["sub_text"].replace("{ALIEN}", alien)
                    if len(args) == 2:
    ...
    def draw(self):
        ...
        main_surfaces = [font_main.render(line, True, main_text_color) for line in wrapped_main]

        # Sous-texte fixe
        if self.sub_text:
            sub_surf = font_sub.render(self.sub_text, True, sub_text_color)
        else:
            sub_surf = font_sub.render("clic gauche pour continuer...", True, sub_text_color)

        # --- Dimensions de la popup ---
        text_width = max(max(s.get_width() for s in main_surfaces), sub_surf.get_width())

        text_height = sum(s.get_height() for s in main_surfaces) + spacing + sub_surf.get_height()
        ...
        pygame.draw.rect(popup, bg_color, (0, 0, width, height), border_radius=border_radius)
    ...
    def do_seq_fire(self):
        ...
        if not self.game.selected_unit.is_moving and len(self.game.units) == 3:
            ...
            self.game.units[-1].position = (
                self.game.selected_unit.position[0] + 20,
                self.game.selected_unit.position[1],
            )
            return True
        return False
