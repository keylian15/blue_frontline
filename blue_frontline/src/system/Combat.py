...
import pygame
from src.config.paths import GREEN_TEAM_PATH_BIG, RED_TEAM_PATH_BIG
from src.core.ExplosionRenderer import ExplosionRenderer
from src.units.Unit import Unit
from src.utils.Utils import load_tileset
...
class Projectile(pygame.sprite.Sprite):
    ...
    def update(self, dt: float):
        ...
        distance_to_target = math.sqrt(
            (self.position[0] - self.target_x) ** 2 + (self.position[1] - self.target_y) ** 2
        )
    ...
    def check_collision(self, target: Unit):
        ...
        distance = math.sqrt((self.position[0] - target_pos[0]) ** 2 + (self.position[1] - target_pos[1]) ** 2)
...
class CombatSystem:
    ...
    def fire_projectile(self, shooter: Unit, target: Unit):
        ...
        if hasattr(shooter, "game") and hasattr(shooter, "team"):
            if (
                shooter.team == "red"
                and hasattr(shooter.game, "achievements_system_rouge")
                and shooter.game.achievements_system_rouge
            ):
                shooter.game.achievements_system_rouge.track_bullet_fired()
            elif (
                shooter.team == "green"
                and hasattr(shooter.game, "achievements_system_vert")
                and shooter.game.achievements_system_vert
            ):
        ...
        try:
            if hasattr(self.game, "sound") and self.game.sound:
                self.game.sound.on_unit_shot(shooter.__class__.__name__, pos=(shooter.position[0], shooter.position[1]))
        except Exception:
            pass
            ...
            if projectile.is_active:
                ...
                if zoom != 1.0:
                    scaled_image = pygame.transform.scale(
                        projectile.image,
                        (
                            max(1, int(projectile.image.get_width() * zoom)),
                            max(1, int(projectile.image.get_height() * zoom)),
                        ),
                    )
                else:
                ...
                screen.blit(
                    scaled_image,
                    (
                        screen_x - scaled_image.get_width() // 2,
                        screen_y - scaled_image.get_height() // 2,
                    ),
                )
            ...
            if mine.is_active:
                ...
                if zoom != 1.0:
                    scaled_image = pygame.transform.scale(
                        mine.image,
                        (
                            max(1, int(mine.image.get_width() * zoom)),
                            max(1, int(mine.image.get_height() * zoom)),
                        ),
                    )
                else:
