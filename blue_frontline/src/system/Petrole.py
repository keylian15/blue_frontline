import pygame
from src.config.game_constants import PETROLE_EVENT, TIME_SPEEDS, TIME_STEP
...
class Petrole:
    ...
    def __init__(self):
        ...
        pygame.time.set_timer(
            self.PETROLE_EVENT, int(TIME_STEP / self.current_speed)
        )  # On fait le timer en fonction de la vitesse du temps
    ...
    def handle_event(self, event: pygame.event, nbPompe: int):
        ...
        if event.type == self.PETROLE_EVENT:
            ...
            self.count += get_gameplay_settings()["OIL_PER_SECOND"] + nbPompe
