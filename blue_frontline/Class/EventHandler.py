...
class EventHandler:
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            self.game.hud.petrole_green.handle_event(
                event, self.game.nbPompePetroliereVert)
            self.game.hud.petrole_red.handle_event(
                event, self.game.nbPompePetroliereRouge)
    ...
    def handle_keydown_events(self, event: pygame.event):
        ...
        # HUD
            ...
            self.game.hud.popup_selection = (
                self.game.hud.popup_selection - 1) % len(self.game.hud.unit_names)
        ...
        # Mines
            ...
            world_x, world_y = self.screen_to_world_coordinates(
    ...
    def handle_mouse_events(self, event: pygame.event):
        ...
        if event.button == get_action_key("SELECT_MOVE"):
                        ...
                        if pos:
                            for i, rect in enumerate(rects):
                                ...
                                if rect.collidepoint(pos):
                                    ...
                                    callback = getattr(
                                        self.game.hud, 'unit_select_callback', None)
            ...
            hud_visible = getattr(self.game, 'hud', None) and getattr(
                self.game.hud, 'show', False)
            ...
            if event.key in seq["restriction"]["keys"]:
                ...
                if seq["count"]:
                    ...
                    if seq["restriction"]["name"] == "spawn_chaloupe":
                        ...
                        self.game.units[-1].position = self.game.get_base_position(
                            "red")
                ...
                if seq["count"]:
                    ...
                    if seq["restriction"]["name"] == "select_chaloupe":
                        ...
                        world_x, world_y = self.screen_to_world_coordinates(
                            pygame.mouse.get_pos())
                        # Chercher une unité à cette position
                        clicked_unit = self.game.find_unit_at_position(
                            world_x, world_y)
    ...
    def spawn_unit(self, config_key: str, team_key: str):
        ...
        if hasattr(self.game, "sound") and self.game.sound: 
            try:
                self.game.sound.on_unit_dropped(
    ...
    def _toggle_qlearning_all_chaloupes(self):
        ...
        chaloupes = [unit for unit in self.game.units if hasattr(
            unit, 'unit_type') and unit.unit_type == 'chaloupe']
        ...
        current_state = chaloupes[0].is_qlearning_enabled(
        ) if chaloupes else False
        ...
        print(
            f"[Q-Learning Debug] Q-Learning {status} pour {len(chaloupes)} chaloupes")
    ...
    def _save_qlearning_progress_all(self):
        ...
        print(
            f"[Q-Learning Debug] Progrès sauvegardé pour {saved_count} chaloupes")
    ...
    def _reset_qlearning_all(self):
        ...
        print(
            f"[Q-Learning Debug] Q-Learning reset pour {reset_count} chaloupes")
